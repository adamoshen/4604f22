...
```{r, eval=FALSE}
ggplot(data=boot_results, aes(x=Trt, y=after_stat(density), colour=method, fill=method)) +
...
Alternatively, see page 229 of [Statistical Computing with R](https://englianhu.files.wordpress.com/2020/10/statistical-computer-with-r-2nd-edition.pdf)
...
- For effective memory consumption, we initialize `boot_estimates` as a vector of zeroes with length
`B`. As we progress through our main loop, we will replace these placeholder zeroes with the
required values.
